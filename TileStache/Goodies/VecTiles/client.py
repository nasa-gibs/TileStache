...
def load_tile_features(lock, host, port, path_fmt, tiles, features):
	...
	while True:
		...
		resp = None
		file = None
		cache_base = "/tmp/stachecache"
		cached_geojson_found = False
		
		# Check for existing GeoJSON here and skip the 'GET' request below,
		# if possible: no need to retrieve MVT twice since vt-geojson already
		# makes that request
		...
		geojson_dir = cache_base + "/geojson/" + layer_name + "/" + str(tile['z']) + "/" + str(tile['x']) + "/"
		geojson_full_path = geojson_dir + str(tile['y']) + ".json"
		if os.path.isfile(geojson_full_path):
			logging.debug('Found cached geojson and using it: ', geojson_full_path)
			cached_geojson_found = True  	
		else:
		...
		with lock:
			...
			if mime_type in ('text/json', 'application/json'):
				...
				# Retrieve and cache MVT if needed
				if not cached_geojson_found:
					# Ensure directory exists
					if not os.path.exists(geojson_dir):
						os.makedirs(geojson_dir)
					
					# Generate GeoJSON (vt-geojson automatically retrieves MVT in the process)
